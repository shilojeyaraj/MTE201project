...
from utils import pixel_distance, pixel_to_mm, linear_fit, r_squared
...
def calibrate_single(img_array: np.ndarray, ax: plt.Axes) -> tuple:
    """
    Run single-point calibration: user clicks 2 stud centers (8.0 mm reference).

    Displays the image, waits for 2 clicks, computes pixel distance and px/mm ratio.
    ...
    -------
    tuple
        (reference_pixel_dist, reference_mm, points_list)
        points_list = [(x1,y1), (x2,y2)] for the two clicked stud centers.
    """
    ax.imshow(img_array)
    ax.set_title("Calibration: Click 2 stud centers (known = 8.0 mm)", fontsize=10)
    ax.axis("on")
    plt.draw()

    # ginput(2) blocks until user clicks 2 points
    ...
    px_per_mm = d_px / STUD_SPACING_MM

    # Annotate the clicked points on the image
    ...
    ax.set_title(
        f"Pixel distance: {d_px:.1f} px  →  {d_px / px_per_mm:.2f} mm  (ref 8.0 mm)\n"
        f"Ratio: {px_per_mm:.2f} px/mm",
        fontsize=9,
    )
    plt.draw()

    return (d_px, STUD_SPACING_MM, [p1, p2])


def calibrate_extended(
    img_array: np.ndarray, ax: plt.Axes, reference_pixel_per_mm: float
) -> dict:
    """
    ...
    ax : plt.Axes
        Axes for display.
    reference_pixel_per_mm : float
        Initial px/mm ratio from single calibration (used for measured mm).
    ...
    for i, span_mm in enumerate(known_mm):
        ...
        p1, p2 = pts[0], pts[1]
        d_px = pixel_distance(p1, p2)
        # Use ratio to get measured mm: measured_mm = d_px / (px_per_mm)
        meas = d_px / reference_pixel_per_mm
